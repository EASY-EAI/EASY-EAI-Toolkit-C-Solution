...
#define PROCESS_ADAPTER_NAME    "adapter"
...
int main(int sdwArgc, char **pcArg)
{
	...
	if(0 == strcmp(pcArg[1], "Main"))
	{
        ...
        if(0 == ini_read_int(RTSP_CLIENT_PATH, "configInfo", "enableChnNum", &chnNum))
        {
            ...
            // 2.3 创建适配器 x 1个
            CreateProcess(PROCESS_ADAPTER_NAME, &st_TaskInfo);
            
            // 2.4 创建分析器 x 1个
            CreateProcess(PROCESS_ANALYZER_NAME, &st_TaskInfo);

            // 2.5 创建Rtsp取流器 x n个，每一个都是一条独立进程
    ...
    /* 适配器器子程进入此分支 */
    } else if(0 == strcmp(pcArg[1], PROCESS_ADAPTER_NAME)) {
        adapterInit();
    /* 分析器子程进入此分支 */
